...
export const ExportExcelDeviceModelList = ({csvData, fileName,requestLoadData}) => {    
    console.log(csvData)
    ...
    let activateLoadingData=async ()=>{
        ...
        return (
            ...
            <ExcelFile element={<button ref={exportExcelButton} className="btn btn-info" type="button" style={{display:"none"}}><i className="text-gray-100 fas fa-file-excel ml-2"></i></button>} filename={fileName}>
                <ExcelSheet data={csvData} name="DeviceModelList">
                    <ExcelColumn label="Name" value="name"/>
                    <ExcelColumn label="Device Mark" value={(col) => col.device_mark_detail.name}/>
                </ExcelSheet>
            </ExcelFile>
                }
            </div>
