...
export const ExportExcelPermissionList = ({ csvData, fileName, loadDataFunction }) => {
    ...
    <div>
        ...
        <ExcelFile filename={fileName} element={<button ref={exportExcelButton} className="btn btn-info" type="button" style={{display:"none"}}><i className="text-gray-100 fas fa-file-excel ml-2"></i></button>}>
            <ExcelSheet data={csvData} name="PermissionList">
                <ExcelColumn label="Name" value="name" />
                <ExcelColumn label="Code name" value="codename" />
                <ExcelColumn label="Content Type" value={(col) => col.content_type_detail.app_label} />
            </ExcelSheet>
        </ExcelFile>
        }
    </div>
