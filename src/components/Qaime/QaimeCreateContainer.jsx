...
import {createQaime,requestQaimeTypeList,requestResponsiblePersonList,requestPersonList,requestStatusList,
...
class QaimeCreateContainer extends React.Component {

    onSubmit = (formData) => {
        this.props.createQaime(formData);
    }
        ...
        return (
            ...
            <>
                ...
                <div className="card shadow mb-4">
                    ...
                    <QaimeCreateReduxForm onSubmit={this.onSubmit}
...
const QaimeForm= ({handleSubmit, error,qaimeTypeListOptions,responsiblePersonListOptions,personListOptions,statusListOptions,
    ...
    return (
        <form onSubmit={handleSubmit}>
            ...
            {createField(null, `qaime_type`,[required],Input,null,null,null,"hidden")}
            ...
            {createField(null, `status`,[required],Input,null,null,null,"hidden")}
      ...
      {fields.map((qaime_detail, index) => (
        <li key={index}>
          <h4>Detail #{index + 1}</h4>
            ...
            {createField('Simcard', `${qaime_detail}simcard`, [], Dropdown,'Simcard',simcardListOptions,'number',null,requestSimcardList,null,null,"")}
            {createField('Project', `${qaime_detail}project`, [required], Dropdown,'Project',projectListOptions,'name',null,requestProjectList,null,null,"")}
            {createField('Company', `${qaime_detail}company`, [required], Dropdown,'Company',companyListOptions,'name',null,requestCompanyList,null,null,"")}
            {createField('Count', `${qaime_detail}.count`,[required],Input,'Count')}
            {createField('Configuration', `${qaime_detail}configuration`, [], Dropdown,'Configuration',configurationListOptions,'name',null,requestConfigurationList,null,null,"")}
            {createField('FWVersion', `${qaime_detail}fw_version`, [], Dropdown,'FWVersion',fwVersionListOptions,'name',null,requestFWVersionList,null,null,"")}
            {createField('Sold (or rent)', `${qaime_detail}.sold_or_rent`,[],Toggle,'Sold (or rent)',null,null,'checkbox')}
...
const QaimeCreateReduxForm = reduxForm({form: 'qaimeCreate', initialValues: {
    name: "",
    qaime_type: 1,
    responsible_person: "",
    recipient: "",
...
export default connect(mapStateToProps, {createQaime,requestQaimeTypeList,requestQaimeListAll,
    requestResponsiblePersonList,requestPersonList,requestStatusList,requestDeviceList,requestAccessoryList,
    requestSimcardList,requestProjectList,requestCompanyList,requestConfigurationList,requestFWVersionList})(QaimeCreateContainer);